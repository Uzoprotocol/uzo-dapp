...
const FarmCards: React.FC = () => {
  ...
  const uzoIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'UZO',
  )

  const uzoPrice =
    uzoIndex >= 0 && stakedValue[uzoIndex]
      ? stakedValue[uzoIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const UZO_PER_BLOCK = new BigNumber(100)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [uzo, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'UZO' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
